...
int comp_int_t(const void *a_ , const void*b_) {
    int_t a = *(int_t*)a_, b = *(int_t*)b_;
...
int main(int argc, char *argv[])
{
    ...
    options.Fact = DOFACT;
    output = pzdistribute(&options, n, &LU, &ScalePermstruct, &Glu_freeable, &LUstruct, &grid);

    options.Fact = FACTORED;
    /* Call the linear equation solver. */
    pzgssvx(&options, &A, &ScalePermstruct, b, ldb, nrhs, &grid, &LUstruct, &SOLVEstruct, berr, &stat, &info);
